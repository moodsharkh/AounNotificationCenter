...
public class MessagesResource {
     ...
     * or with status 500 (Internal Server Error) if the messages couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/messages")
    @Timed
    public ResponseEntity<Messages> updateMessages(@RequestBody Messages messages) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, messages.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Messages> getMessages(@PathVariable Long id) {
        ...
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(messages));
    }
